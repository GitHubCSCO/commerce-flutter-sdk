...
void main() async {
  ...
  final currentSession = await sessionService.getCurrentSession();
  switch (currentSession) {
    case Success(value: var value):
      {
        value?.fulfillmentMethod = 'pickUp';
        final updatedSession =
            await sessionService.patchSession(value ?? Session());

        switch (updatedSession) {
          case Success(value: final newValue):
            {
              print(newValue?.fulfillmentMethod);

              final newCurrentSession =
                  await sessionService.getCurrentSession();
              switch (newCurrentSession) {
                case Success(value: final anotherValue):
                  {
                    print(anotherValue?.fulfillmentMethod);
                  }
