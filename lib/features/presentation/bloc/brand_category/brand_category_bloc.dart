...
class BrandCategoryBloc extends Bloc<BrandCategoryEvent, BrandCategoryState> {
  ...
  BrandCategoryBloc({required BrandCategoryUseCase brandCategoryUseCase})
    ...
    on<BrandCategoryLoadEvent>((event, emit) => _onBrandCategoryLoadEvent(event, emit));
  }

  Future<void> _onBrandCategoryLoadEvent(BrandCategoryLoadEvent event, Emitter<BrandCategoryState> emit) async {
    ...
    if(event.brandCategory!=null){
      brandCategoriesQueryParameter.categoryId = event.brandCategory?.categoryId;
      brandCategoriesQueryParameter.productListPagePath = event.brandCategory?.productListPagePath;
      final response = await _brandCategoryUseCase.getBrandCategorySubCategories(
        brandCategoriesQueryParameter
      );
      switch (response) {
        case Success(value: final data):{
          if(data?.subCategories!=null){
            var brandCatagories = data?.subCategories!.map((e) => 
              BrandCategory.mapCategoryToBrandCategory(e)!
            ).toList() ?? [];
      ...
      final response = await _brandCategoryUseCase.getBrandCategories(
        brandCategoriesQueryParameter
      );
      switch (response) {
        case Success(value: final data):{
  ...
  Future<GetBrandSubCategoriesResult?> onSelectBrandCategory(BrandCategory? brandCategory) async {
    ...
    final response = await _brandCategoryUseCase.getBrandCategorySubCategories(brandCategoriesQueryParameter);
